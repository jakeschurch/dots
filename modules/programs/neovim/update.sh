...
usage () {
  echo "Usage: update.sh fromfile <file>"
  echo "       update.sh <function>"
}
...
lookup() {
	...
	gh api --cache "5h" "/repos/$plugin/branches/$ref" |
		jq '{name: $plugin, rev: .commit.sha, ref: $ref}' --arg plugin "$plugin" --arg ref "$ref" |
		tee "$tmpfile"
}
