-- Cursorrules Interface for codecompanion
local M = {}
  ...
  for _, file in ipairs(scan_files) do
    if vim.fn.isdirectory(file) == 0 then
      ...
      if #patterns > 0 then
        ...
        table.insert(
          files,
          { file = file, patterns = patterns, content = filtered_content }
        )
      end
    end
  end
...
function M.init(context, opts)
  ...
  local file_contents =
    M.get_matching_file_contents(rule_files, file_name_from_root_dir)
