      ...
      finder = finders.new_table({
        ...
        entry_maker = function(file)
          ...
          for i, msg in ipairs(result.messages) do
            ...
            table.insert(
              msg_previews,
              string.format("%d. %s: %s", i, role_display, preview)
            )
          end
          ...
          return {
            ...
            display = string.format(
              "%s (%s)",
              name,
              result.timestamp or "unknown time"
            ),
            ordinal = content,
            filename = name,
      ...
      attach_mappings = function(prompt_bufnr)
        actions.select_default:replace(function()
          ...
          local confirm = vim.fn.input(
            string.format("Delete session '%s'? [y/N] ", selection.filename)
          )
          ...
          if success then
            util.notify(
              string.format(
                "Session deleted: %s",
                vim.fn.fnamemodify(selection.value, ":~")
              )
            )
          else
            util.notify(
...
local function callback(_, args)
  ...
  if args and args ~= "" then
    ...
    -- Confirm deletion
    local confirm =
      vim.fn.input(string.format("Delete session '%s'? [y/N] ", args))
    if confirm:lower() ~= "y" then
      util.notify("Session deletion cancelled")
      return
    end
    ...
    if success then
      ...
      util.notify(
        string.format("Failed to delete session: %s", err or "unknown error"),
        vim.log.levels.ERROR
      )
    end
  else
