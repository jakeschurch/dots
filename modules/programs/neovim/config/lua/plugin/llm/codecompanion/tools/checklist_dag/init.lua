...
local dag_manager_module = require("plugin.llm.codecompanion.tools.checklist_dag.dag_manager")
local dag_formatter_module = require("plugin.llm.codecompanion.tools.checklist_dag.dag_formatter")
local dag_executor = require("plugin.llm.codecompanion.tools.checklist_dag.dag_executor")
local storage_module = require("plugin.llm.codecompanion.tools.checklist_dag.storage")
...
local ChecklistDagCreateTool = {
	...
	cmds = {
		...
		function(agent, args, input, cb)
			...
			if not tasks_input or type(tasks_input) ~= "table" or #tasks_input == 0 then
				cb({
					status = "error",
					data = {},
			...
			local independent_tasks = manager.get_independent_tasks and manager:get_independent_tasks(tasks_data) or {}
			...
			if #independent_tasks > 0 then
				...
				dag_executor.execute_tasks_parallel(tasks_to_execute, parent_chat, function(parallel_results)
					local checklist, err = manager.create_checklist
						and manager:create_checklist(goal, tasks_data, subject, body, parallel_results)
...
local ChecklistDagStatusTool = {
	...
	cmds = {
		...
		function(agent, args, input, cb)
			...
			local checklist, err = manager.get_checklist and manager:get_checklist(checklist_id) or nil
			if not checklist or type(checklist) ~= "table" then
				cb({
					status = "error",
					data = {},
...
local ChecklistDagListTool = {
	...
	cmds = {
		...
		function(agent, args, input, cb)
			...
			local manager = system.manager
			local all_checklists = manager.get_all_checklists and manager:get_all_checklists() or {}
...
local ChecklistDagCompleteTaskTool = {
	...
	cmds = {
		...
		function(agent, args, input, cb)
			...
			if not cb or type(cb) ~= "function" then
				print("ChecklistDagCompleteTaskTool: callback missing or not a function")
				return
			end
			local system = get_shared_dag_system()
			if not system or not system.manager then
