...
function CodeExtractor:get_symbol_data(bufnr, row, col)
  ...
  if not parser then
    vim.notify(
      "Can't initialize tree-sitter parser for buffer id: " .. bufnr,
...
function CodeExtractor:validate_lsp_params(bufnr, method)
  if not (bufnr and method) then
    vim.notify(
      "Unable to call lsp. Missing bufnr or method. buffer="
        .. bufnr
        .. " method="
        .. method,
      vim.log.levels.WARN
...
function CodeExtractor:execute_lsp_request(bufnr, method)
  local position_params =
    vim.lsp.util.make_position_params(0, vim.lsp.client.offset_encoding)

  local results_by_client, err = vim.lsp.buf_request_sync(
    bufnr,
    method,
    position_params,
    self.lsp_timeout_ms
...
function CodeExtractor:process_single_range(uri, range)
  ...
  local data =
    self:get_symbol_data(target_bufnr, range.start.line, range.start.character)
  if data then
      ...
      function(_, action, _)
        ...
        if type == "edit" then
          ...
          if code_extractor.lsp_methods[type] then
            code_extractor:call_lsp_method(
              bufnr,
              code_extractor.lsp_methods[type]
            )
