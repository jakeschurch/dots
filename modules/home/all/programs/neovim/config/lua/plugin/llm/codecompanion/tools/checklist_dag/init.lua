...
local dag_manager_module =
  require("plugin.llm.codecompanion.tools.checklist_dag.dag_manager")
local dag_formatter_module =
  require("plugin.llm.codecompanion.tools.checklist_dag.dag_formatter")
local dag_executor =
  require("plugin.llm.codecompanion.tools.checklist_dag.dag_executor")
local storage_module =
  require("plugin.llm.codecompanion.tools.checklist_dag.storage")
...
local ChecklistDagCreateTool = {
  ...
  cmds = {
    ...
    function(agent, args, input, cb)
      local goal = args and args.goal
      local tasks_input = args and args.tasks or {}
      local subject = args and args.subject
      local body = args and args.body

      if not cb or type(cb) ~= "function" then
        print("ChecklistDagCreateTool: callback missing or not a function")
        return
      end
      ...
      if not system or not system.manager then
        cb({
          status = "error",
          data = {},
          message = "DAG system manager unavailable",
        })
        return
      end
      ...
      for i, task_input in ipairs(tasks_input) do
        ...
        elseif type(task_input) == "table" then
          ...
          cb({
            status = "error",
            data = {},
            message = "Invalid task input type",
          })
          return
        end
      end
      ...
      local independent_tasks = manager.get_independent_tasks
          and manager:get_independent_tasks(tasks_data)
        or {}
      ...
      if #independent_tasks > 0 then
        ...
        local parent_chat =
          require("codecompanion.strategies.chat").buf_get_chat(parent_bufnr)

        -- Execute independent tasks in parallel
        dag_executor.execute_tasks_parallel(
          tasks_to_execute,
          parent_chat,
          function(parallel_results)
            local checklist, err = manager.create_checklist
              and manager:create_checklist(
                goal,
                tasks_data,
                subject,
                body,
            ...
            if not checklist or type(checklist) ~= "table" then
              cb({
                status = "error",
                data = {},
                message = err or "Checklist creation failed",
              })
              return
            end
            cb({
        ...
        local checklist, err = manager.create_checklist
            and manager:create_checklist(goal, tasks_data, subject, body, {})
          or nil
        if not checklist or type(checklist) ~= "table" then
          cb({
            status = "error",
    ...
    ["function"] = {
      ...
      parameters = {
        ...
        properties = {
          ...
          subject = {
            type = "string",
            description = "Commit subject (summary/title)",
          },
          body = {
            type = "string",
            description = "Commit body (detailed explanation)",
          },
        },
  ...
  output = {
    success = function(tool, agent, cmd, stdout)
      ...
      if response_data and response_data.checklist then
        ...
        local user_formatted =
          dag_formatter:format_checklist(checklist, progress)
        ...
        if not vim.tbl_isempty(parallel_results) then
          ...
          for task_idx, result in pairs(parallel_results) do
            local truncated = #result > 80 and (result:sub(1, 77) .. "...")
              or result
            user_formatted = user_formatted
    ...
    error = function(tool, agent, cmd, stderr)
      ...
      agent.chat:add_tool_output(
        tool,
        string.format("**Checklist DAG Tool Error**: %s", error_msg)
      )
    end,
    ...
    rejected = function(tool, agent, cmd)
      agent.chat:add_tool_output(
        tool,
        "**Checklist DAG Tool**: User declined to execute the operation"
      )
    end,
  },
...
local ChecklistDagStatusTool = {
  ...
  cmds = {
    ...
    function(agent, args, input, cb)
      args = args or {}
      if not cb or type(cb) ~= "function" then
        print("ChecklistDagStatusTool: callback missing or not a function")
        return
      end
      ...
      local checklist, err = manager.get_checklist
          and manager:get_checklist(checklist_id)
        or nil
      if not checklist or type(checklist) ~= "table" then
        cb({
          status = "error",
          data = {},
          message = err or "Checklist not found",
        })
        return
      end
...
- checklist_dag_status({ checklist_id = "2" }) -- status of DAG checklist with ID 2
  ...
  output = {
    ...
    error = function(tool, agent, cmd, stderr)
      ...
      agent.chat:add_tool_output(
        tool,
        string.format("**Checklist DAG Status Tool Error**: %s", error_msg)
      )
    end,
    ...
    rejected = function(tool, agent, cmd)
      agent.chat:add_tool_output(
        tool,
        "**Checklist DAG Status Tool**: User declined to execute the operation"
      )
    end,
  },
  args = {},
...
local ChecklistDagListTool = {
  ...
  cmds = {
    ...
    function(agent, args, input, cb)
      if not cb or type(cb) ~= "function" then
        print("ChecklistDagListTool: callback missing or not a function")
        return
      end
      local system = get_shared_dag_system()
      if not system or not system.manager then
      ...
      local all_checklists = manager.get_all_checklists
          and manager:get_all_checklists()
        or {}
      if type(all_checklists) ~= "table" then
        cb({
          status = "error",
          data = {},
          message = "Checklist list unavailable",
        })
        return
      end
      cb({
...
- checklist_dag_list()
  ...
  output = {
    ...
    error = function(tool, agent, cmd, stderr)
      ...
      agent.chat:add_tool_output(
        tool,
        string.format("**Checklist DAG List Tool Error**: %s", error_msg)
      )
    end,
    ...
    rejected = function(tool, agent, cmd)
      agent.chat:add_tool_output(
        tool,
        "**Checklist DAG List Tool**: User declined to execute the operation"
      )
    end,
  },
  args = {},
...
local ChecklistDagCompleteTaskTool = {
  ...
  cmds = {
    ...
    ---@param cb function
    function(agent, args, input, cb)
      local checklist_id = args and args.checklist_id
      local task_id = args and args.task_id
      local subject = args and args.subject
      local body = args and args.body

      if not cb or type(cb) ~= "function" then
        print(
          "ChecklistDagCompleteTaskTool: callback missing or not a function"
        )
        return
      end
      local system = get_shared_dag_system()
      ...
      local success, msg = manager.complete_task
        and manager:complete_task(agent, checklist, task_id, subject, body)
      if not success then
        cb({
          status = "error",
          data = {},
          message = msg or "Failed to complete task",
        })
        return
      end
      cb({
    ...
    ["function"] = {
      ...
      parameters = {
        ...
        properties = {
          ...
          task_id = {
            type = "string",
            description = "Task ID to mark complete (must be in progress)",
          },
          subject = {
            type = "string",
            description = "Commit subject (summary/title)",
  ...
  output = {
    success = function(tool, agent, cmd, stdout)
      ...
      if checklist then
        ...
        local user_formatted =
          dag_formatter:format_task_completion(checklist, next_idx, next_task)
        agent.chat:add_tool_output(tool, llm_output, user_formatted)
      else
        agent.chat:add_tool_output(tool, "No DAG checklist data available")
      end
    ...
    error = function(tool, agent, cmd, stderr)
      ...
      agent.chat:add_tool_output(
        tool,
        string.format(
          "**Checklist DAG Complete Task Tool Error**: %s",
          error_msg
        )
      )
    end,
