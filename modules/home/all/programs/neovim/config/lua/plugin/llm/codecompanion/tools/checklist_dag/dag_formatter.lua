...
local dag_types =
  require("plugin.llm.codecompanion.tools.checklist_dag.dag_types")
...
function DagChecklistFormatter:format_checklist(checklist, progress)
  ...
  if #checklist.tasks == 0 then
    ...
    local display_order = checklist.execution_order
        and #checklist.execution_order > 0
  ...
  if
    checklist.dependency_graph
    and not vim.tbl_isempty(checklist.dependency_graph)
  then
    output = output .. "\n\nDependency graph:"
  ...
  if checklist.log and #checklist.log > 0 then
    ...
    for _, entry in ipairs(sorted_log) do
      ...
      if entry.completed_task_ids and #entry.completed_task_ids > 0 then
        table.insert(
          details,
          "completed: " .. table.concat(entry.completed_task_ids, ",")
        )
      end
      ...
      if
        entry.parallel_results and not vim.tbl_isempty(entry.parallel_results)
      then
        local results = {}
        for task_idx, result in pairs(entry.parallel_results) do
          local truncated = #result > 30 and (result:sub(1, 27) .. "...")
            or result
      ...
      local detail_str = #details > 0
          and (" (" .. table.concat(details, "; ") .. ")")
        or ""
      output = output
        .. string.format(
          "\n  %s %s%s",
...
function DagChecklistFormatter:format_checklist_summary(checklist, progress)
  local blocked_str = progress.blocked > 0
      and string.format(", %d blocked", progress.blocked)
    or ""
...
function DagChecklistFormatter:format_checklist_list(checklists_with_progress)
  ...
  for _, item in ipairs(sorted_summaries) do
    output = output
      .. self:format_checklist_summary(item.checklist, item.progress)
      .. "\n"
  end
...
function DagChecklistFormatter:format_task_completion(
  checklist,
  next_task_idx,
  next_task
)
  if next_task then
    local deps_info =
      self:format_dependencies(next_task, next_task_idx, checklist)
    return string.format(
      "Next: %d. %s%s",
      next_task_idx,
      next_task.text,
