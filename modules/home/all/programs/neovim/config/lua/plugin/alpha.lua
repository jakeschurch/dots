...
local function add_button(filepath, shortcut_key, text)
	...
	return startify.button(shortcut_key, text_to_show, ":e " .. filepath .. "<CR>")
end

local function add_icon_button(filepath, shortcut_key, text)
	local icon_text = (get_file_icon(filepath) or "") .. "\t" .. (text or filepath)
...
local function get_git_repos(directory)
	...
	local handle = io.popen("fd .git$ " .. directory .. ' -d 2 -u -H  -t d -x stat -c "%Y %n" | sort -n -r | head -n 5')
	if handle then
		for line in handle:lines() do
			local last_modified, path = line:match("(%d+) (.+)")
			if last_modified and path then
				table.insert(repos, { path = path, last_modified = tonumber(last_modified) })
			end
		end
		handle:close()
	end
	...
	val = {
		{ type = "text", val = "Bookmarks", opts = { hl = "SpecialComment" } },
		{
			...
			val = {
				...
				add_icon_button("~/.dots/modules/programs/neovim/default.nix", "N", "nvim config"),
				add_icon_button("~/.dots/modules/development.nix", "d", "dev env"),
				add_icon_button("~/.dots/modules/programs/git/default.nix", "g", "git config"),
			},
			opts = { hl = "SpecialComment", position = "center" },
		},
	},
...
vim.keymap.set("n", "<leader>e", ":Alpha<CR>", { noremap = true, silent = true })
