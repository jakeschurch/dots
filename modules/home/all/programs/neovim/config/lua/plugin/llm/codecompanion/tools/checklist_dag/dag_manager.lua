  ...
  for i, task_data in ipairs(tasks_data) do
    ...
    for _, dep in ipairs(deps) do
      if dep < 1 or dep > num_tasks then
        return false,
          string.format(
            "Invalid dependency: task %d depends on non-existent task %d",
            i,
            dep
          )
      end
      if dep == i then
        return false,
          string.format(
            "Self-dependency detected: task %d depends on itself",
            i
          )
      end
    end
...
function DagChecklistManager:get_ready_tasks(checklist)
  ...
  for i, task in ipairs(checklist.tasks) do
    if
      ...
      or task.status == dag_types.TASK_STATUS.BLOCKED
    then
...
function DagChecklistManager:create_checklist(
  goal,
  tasks_data,
  subject,
  body,
...
function DagChecklistManager:complete_task(
  agent,
  checklist,
  task_id,
  subject,
