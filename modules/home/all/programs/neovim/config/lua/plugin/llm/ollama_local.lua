...
M.parse_curl_args = function(self, prompt_opts)
	local options = {
		...
		temperature = prompt_opts.temperature or (self.options and self.options.temperature) or 0,
	}
	...
	return {
		...
		body = {
			model = M.model,
			messages = require("avante.providers").copilot.parse_messages(prompt_opts),
			options = options,
			stream = true,
		},
	}
