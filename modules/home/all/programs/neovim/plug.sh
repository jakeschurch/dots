...
# Dependency check
for dep in gh jq nix-prefetch-url; do
...
CACHE_DIR="${TMP}/cache"
MAX_JOBS=4
LOG_LEVEL="${LOG_LEVEL:-info}" # Can be "debug", "info", "warn", or "error"
...
update_file() {
  ...
  declare -a args
  mapfile -t args < <(jq -r '.[] | [.owner, .repo] | join(":")' "$file")
...
generate_file() {
  ...
  mapfile -t json_files < <(find "$TMP" -type f -name "*.json")
