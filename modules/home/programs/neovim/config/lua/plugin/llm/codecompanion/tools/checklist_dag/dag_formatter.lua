...
local dag_types = require("plugin.llm.codecompanion.tools.checklist_dag.dag_types")
...
function DagChecklistFormatter:format_checklist(checklist, progress)
	...
	if checklist.dependency_graph and not vim.tbl_isempty(checklist.dependency_graph) then
		...
		for task_idx, dependents in pairs(checklist.dependency_graph) do
			if #dependents > 0 then
				output = output .. string.format("\n  %d enables: %s", task_idx, table.concat(dependents, ","))
			end
		end
	end
	...
	if checklist.log and #checklist.log > 0 then
		...
		for _, entry in ipairs(sorted_log) do
			...
			output = output
				.. string.format("\n  %s %s%s", os.date("%m/%d %H:%M", entry.timestamp), entry.action, detail_str)
		end
	end

	if progress then
		output = output .. string.format("\n\nProgress: %d/%d complete", progress.completed, progress.total)
